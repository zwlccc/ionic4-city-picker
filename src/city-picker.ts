...
// import { updateDate } from './city-picker-util';
...
import { CityPickerColumn, CityData } from './city-picker.model';
...
export class CityPicker implements AfterContentInit, ControlValueAccessor  {
  private cityIds = 0;
  ...
  private provinceCol?: number = 0;
  private cityCol?: number = 0;
  private regionCol?: number = 0;
  ...
  private convertDuration(durationObject: any, type: any) {
  ...
  private async valiCity(picker: HTMLIonPickerElement) {
    ...
    if(cityCol && this.provinceCol != provinceCol.selectedIndex){
      ...
      let cityColData: CityPickerColumn[] = this.citiesData[<number>provinceCol.selectedIndex].children;
    ...
    if(regionCol && (this.cityCol != cityCol.selectedIndex || this.provinceCol != provinceCol.selectedIndex)){
      let regionData: CityPickerColumn[] = this.citiesData[<number>provinceCol.selectedIndex].children[<number>cityCol.selectedIndex].children;
  ...
  private async getString(newData:any) {
